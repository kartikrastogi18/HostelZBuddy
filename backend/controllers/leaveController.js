...
import { Leave, HostelSetting, Payment, User } from "../models/index.js";
...
export const applyLeave = async (req, res) => {
  try {
    ...
    if (total_days > 4) {
      return res.status(400).json({
        message: "Leave cannot exceed 4 days",
      });
    }
    ...
    if (oldDue) {
      return res.status(400).json({
        message: `Clear dues of ${targetMonth}/${targetYear} before applying leave`,
      });
    }
  ...
  try {
    ...
    if (status === "approved") {
      const setting = await HostelSetting.findOne({
        where: { hostel_id: leave.hostel_id },
      });
      ...
      const per_day_cost =
        Math.floor(setting.monthly_charge / 30);

      leave.refund_amount =
        leave.total_days * per_day_cost;
    }
...
export const getHostelLeaves = async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: "Server error fetching hostel leaves",
    });
  }
};
